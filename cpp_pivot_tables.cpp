...
std::vector<std::string>& pivot_index, std::string& pivot_val,
...
*/
    ...
    // to help ensure that our final output is correct. (Not sure
    // whether this is strictly necessary to specify, but it 
    // doesn't hurt.)
    ...
    std::map<std::vector<std::string>, Pivot_Vals> pivot_map;
    //  {0.0, 0.0}
    ...
    for (CSVRow &row : reader)
    {
        ...
        {
            std::vector<std::string> pivot_index_vals{};
            for (std::string &variable : pivot_index)
            {
                pivot_index_vals.push_back(row[variable].get());
            }
    ...
    std::vector<std::string> header_row = pivot_index;
...
std::vector<std::string> pivot_index {"CARRIER", "ORIGIN", "REGION"};
